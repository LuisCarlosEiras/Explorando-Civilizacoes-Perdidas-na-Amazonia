...
def comparar_metodos(y_true, y_pred_1, y_pred_2):
    ...
    metricas = {
        ...
        'Matriz Confusão Método 1': cm_1.tolist(),  # Converter para lista para serialização JSON
        'Matriz Confusão Método 2': cm_2.tolist()   # Converter para lista para serialização JSON
    }
...
def converter_para_tipos_nativos(obj):
    """
    ...
    if isinstance(obj, np.integer):
        ...
        return {k: converter_para_tipos_nativos(v) for k, v in obj.items()}
    elif isinstance(obj, list):
        return [converter_para_tipos_nativos(i) for i in obj]
    else:
        return obj
...
def demonstrar_previsao_coordenadas():
    ...
    for regiao in regioes:
        ...
        if len(sitios_alta_confianca) > 0:
            ...
            resumo = {
                'Região': regiao,
                'Total de Pontos Testados': int(len(y_test)),
                'Sítios Reais': int(np.sum(y_test)),
                'Previsões Método 1': int(np.sum(y_pred_1)),
                'Previsões Método 2': int(np.sum(y_pred_2)),
                'Sítios Alta Confiança': int(len(sitios_alta_confianca)),
                'Acurácia Método 1': float(metricas['Acurácia Método 1']),
                'Acurácia Método 2': float(metricas['Acurácia Método 2']),
                'Concordância': float(metricas['Concordância entre métodos'])
            }
